...
export const billItemSchema = z.object({
  ...
  type: z.enum(['food', 'drink', 'dessert']).describe(
    'Type of the bill item (food, drink, or dessert)',
  ),
});
...
export const consumptionItemSchema = z.object({
  ...
  proportion: z.number().describe(
    'Proportion of the item consumed (e.g., 0.5 for half)',
  ),
});
...
export const userConsumptionSchema = z.object({
  ...
  consumption: z.array(consumptionItemSchema).describe(
    'List of items consumed by this user',
  ),
});
